...
export class WorkerLogic {
  ...
  start() {
    ...
    this.remoteServer.subscribe("maze", (data: TJSON) => {
      ...
      if (retParse.isSuccess) {
        ...
        this.logger.err(
          `Could not understand the maze sent from the server: ${retParse.error}`
        );
      }
    });
    this.remoteServer.subscribe("simulate", (data: TJSON) => {
      ...
      if (retParse.isSuccess) {
        ...
        this.logger.err(
          `Could not understand the simulation query sent from the server: ${retParse.error}`
        );
      }
    });
  }
  ...
  stop() {
    ...
    this.logger.log(
      this.isLocalWorker ? "Local worker stopped" : "Worker stopped"
    );
    process.exit(0);
  }
  ...
  createWorlds() {
    if (this._maze) {
      ...
      for (let i = 0; i < this.nbThreads; i++) {
        this._worlds.push(World.createRandomWorld(this._maze, 20000));
      }
    }
  }
