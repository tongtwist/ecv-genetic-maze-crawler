...
export class Maze implements IMaze {
    ...
    private _removeRandomWall(idx: number) {
        ...
        for (let sideIdx = 0; sideIdx <= 3; sideIdx++) {
            if (cell.walls[sideIdx]) {
                ...
                if (
                    neighborI >= 0 &&
                    neighborI < this._nbCols &&
                    neighborJ >= 0 &&
                    neighborJ < this._nbRows
                ) {
        ...
        if (removableWalls.length > 0) {
            const randomSideIdx = Math.floor(
                Math.random() * removableWalls.length
            )
    ...
    static fromJSON(nbCols: number, nbRows: number, json: TMazeJson): IMaze {
        const walls: [boolean, boolean, boolean, boolean][] = json.g.map(
            (flags: number) => Cell.wallsFromJSON(flags)
        )
